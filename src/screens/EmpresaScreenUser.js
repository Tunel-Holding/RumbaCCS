...
const renderSocialCircles = () => {
  ...
  // Pantalla de carga con fade-out usando LoadingScene (solo en el primer load)
  const [showLoader, setShowLoader] = useState(true);
  useEffect(() => {
    if (!loading) {
      // Espera el fade-out antes de ocultar el loader
      setTimeout(() => setShowLoader(false), 400); // 400ms para animación
    }
  }, [loading]);
  ...
  return (
    <SafeAreaView style={[styles.container, { paddingTop: insets.top }]}> 
      ...
      {/* Loader overlay solo en el primer load */}
      {showLoader && (
        <LoadingScene visible={!loading} text="Cargando datos..." />
      )}
