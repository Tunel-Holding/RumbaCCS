...
export default function BuyScreen() {
  ...
  // Full-screen loading overlay only on the first load
  ...
  // Control de interacción del usuario para pausar autoplay durante el arrastre
  const [isInteracting, setIsInteracting] = useState(false);

  // No actualizamos el índice en onScroll para evitar parpadeos; lo haremos en onMomentumScrollEnd
  const handleScroll = () => {};

  const goTo = (index) => {
    if (!eventImages || eventImages.length === 0) return;
    const len = eventImages.length;
    const next = ((index % len) + len) % len; // normaliza negativo/overflow
    // No seteamos activeIndex aquí para evitar parpadeo; se establece en onMomentumScrollEnd
  ...
  useEffect(() => {
    ...
    if (eventImages && eventImages.length > 1 && !isInteracting) {
      autoplayRef.current = setInterval(() => {
        goNext();
      }, 4500);
    }
    return () => clearInterval(autoplayRef.current);
  }, [activeIndex, isInteracting, eventImages?.length, slideWidth]);
...
if (loading && showInitialOverlay) {
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: '#0f172a' }]}>
      <StatusBar barStyle="light-content" backgroundColor="#0f172a" />
      <LoadingScene variant="skeleton-cards" message="Buscando detalles del evento..." />
    </SafeAreaView>
  );
}
  ...
  <View style={[styles.carouselEnhancedWrapper, styles.fullBleed]}>
          ...
          <ScrollView
            ...
            onMomentumScrollEnd={(e) => {
              ...
              // Reanudar autoplay tras finalizar el deslizamiento: el efecto se encargará
