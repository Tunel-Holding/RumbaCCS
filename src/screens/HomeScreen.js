...
import LoadingOverlay from '../components/LoadingScene';
...
// }, [filter, userLocation, search]);
  ...
  // Nota: No maneja el spinner de pantalla completa; el caller decide si mostrarlo.
  ...
  // Solo mostramos el loader de pantalla completa la primera vez que la pantalla entra en foco.
  const didFirstFocus = useRef(false);
  useFocusEffect(
    useCallback(() => {
      let isActive = true;
      const checkLoginAndRefresh = async () => {
        const firstTime = !didFirstFocus.current;
        ...
        if (token) {
          ...
          await refreshEmpresaData(); // No muestra loader de pantalla completa
        } else {
  ...
  if (loading) {
    return (
      <View style={[styles.container, { backgroundColor: '#0f172a', flex: 1 }]}> 
        ...
        <LoadingOverlay variant="skeleton-cards" message="Buscando eventos increíbles..." />
      </View>
    );
  }
