...
export default function BuyScreen() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      const loggedIn = !!(token && token.trim().length > 0);
    ...
    checkSession();
  }, [loginVisible]); // Solo se re-ejecuta cuando cambia loginVisible (no isLogged, para evitar bucle)
    ...
    <SafeAreaView style={[styles.safeArea, { paddingTop: insets.top, paddingBottom: 0 }]}>
      ...
      <ScrollView style={[styles.container, { paddingTop: 12, paddingBottom: 0 }]} contentContainerStyle={{ paddingBottom: 32 + insets.bottom }}>
        ...
        <View style={styles.buttonContainer}>
          {/* Botón Principal de Reserva - OCULTO TEMPORALMENTE */}
          {/*
