...
// Formateador consistente para Venezuela: DD/MM/YY
// Evita depender de Intl en RN y asegura el formato esperado.
// Si prefieres año completo, cambia `String(d.getFullYear()).slice(-2)` por `d.getFullYear()`.
const formatDateDDMMYY = (input) => {
  if (!input) return 'Fecha no definida';
  try {
    let d;
    // Manejar cadenas tipo 'YYYY-MM-DD' como fecha local para evitar desfases por zona horaria
    ...
    if (isNaN(d)) return 'Fecha no definida';
      ...
      try {
        ...
        const eventosTransformados = resultadosRaw.map(ev => ({
          ...
          date: ev.fecha_evento
            ? formatDateDDMMYY(ev.fecha_evento)
            : ev.creado_en
            ? formatDateDDMMYY(ev.creado_en)
            : "Fecha no definida",
